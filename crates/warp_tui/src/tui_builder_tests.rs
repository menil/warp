...
#[test]
fn voice_input_border_pulses_between_cyan_overlay_2_and_cyan() {
    let theme = light_theme();
    let builder = TuiUiBuilder {
        warp_theme: theme.clone(),
    };
    let cyan = ThemeFill::from(theme.terminal_colors().normal.cyan);
    let cyan_overlay_2: Color =
        CoreFill::from(theme.background().blend(&cyan.with_opacity(50))).into();
    let cyan: Color = CoreFill::from(cyan).into();
